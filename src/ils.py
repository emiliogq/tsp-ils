...
class ILS:
    ...
    def local_search(self, solution, cost, threshold):
        i = 0
        while( i < threshold):
            new_solution = stochastic_two_opt(solution)
            new_cost = self.tour_cost(new_solution)
            if (new_cost < cost):
    ...
    def perturb(self,solution, history:list):
    ...
    def run(self, data, threshold = 50, iterations: int = 1000, degradation_grace_period = -1):
        ...
        base_cost = self.tour_cost(base_solution)
        # Getting local optima
        best_solution, best_cost = base_solution, base_cost = self.local_search(base_solution, base_cost, threshold)
        i = 0
        while ( i < iterations):
            solution, cost = self.perturb(base_solution, history)
            solution, cost = self.local_search(solution, cost, threshold)        
            if (cost < best_cost):
...
def double_bridge_move(solution):
    slice_length = int(len(solution) / 4)
